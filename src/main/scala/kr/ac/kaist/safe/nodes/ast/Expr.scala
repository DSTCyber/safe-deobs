...
/**
 * Internally generated by the deobfuscation phase.
 *
 * Replaces dead code expressions and is removed by the \c SimplifyWalker. It
 * does not appear in the JavaScript source text.
 */
case class EmptyExpr(
  ...
  override def =~(that: ASTNode): Boolean = (this, that) match {
    case (EmptyExpr(_), EmptyExpr(_)) => true
    case _ => false
  }
}
